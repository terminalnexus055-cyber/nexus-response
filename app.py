...
Triage options available: {opt1}, {opt2}, {opt3}

Rules you must follow:
- Keep responses short and conversational — this is SMS
- Never make up information about the business
- For bookings: collect name, preferred date and time
- For emergencies: collect name, briefly confirm urgency, then say merchant is being alerted
- For general queries: answer helpfully but stay within scope
- Never break character
- End booking collection with: "Thank you! I've noted your appointment request. The team will confirm shortly."
- End emergency with: "I understand. I'm alerting the team right now. Someone will call you back immediately."
"""
...
def process_lead(to_num, from_num):
    try:
        ...
        menu = build_triage_menu(merchant)
        send_sms(from_num, to_num, menu)
...
def process_reply(to_num, from_num, body):
    try:
        ...
        lead_result = supabase.table("leads").select("*").eq("lead_phone", from_num).eq("client_id", merchant["id"]).order("created_at", desc=True).limit(1).execute()
        ...
        # Stage: waiting for triage selection
        ...
        # Stage: active AI conversation
            ...
            if "confirmed shortly" in ai_reply.lower() or "call you back" in ai_reply.lower():
                new_stage = "complete"
